"""
This file is adapted from https://github.com/VITA-Group/FSGS
"""
import os
import numpy as np
import sys
...
def pipeline(base_path, n_views):
    os.chdir(base_path)
    view_path = str(n_views) + '_views' if n_views > 0 else 'all_views'
    ...
    train_img_list = [c for c in img_list]
    if n_views > 0:
...
def main():
    parser = argparse.ArgumentParser(description='Custom Dataset MVS Pipeline')
    parser.add_argument('--scene_dir', type=str, help='Base path to the scene directory', required=True)
    parser.add_argument('--n_views', type=int, help='Number of views sampled uniformly', default=-1, required=False)
    args = parser.parse_args()

    base_path = args.scene_dir
    n_views = args.n_views

    pipeline(base_path, n_views)
