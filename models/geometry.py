...
class VolumeDensity(BaseImplicitGeometry):
    def setup(self):
        ...
        if self.config.xyz_encoding_config.get("finest_resolution", None):
            finest_resolution = self.config.xyz_encoding_config["finest_resolution"]
            base_resolution = self.config.xyz_encoding_config["base_resolution"]
            n_levels = self.config.xyz_encoding_config["n_levels"]
            self.config.xyz_encoding_config["per_level_scale"] = float(
                np.exp2(np.log2(finest_resolution / base_resolution) / (n_levels - 1))
        ...
        if self.grad_type == "finite_difference":
            if isinstance(self.finite_difference_eps, float):
                ...
                if grid_size != self._finite_difference_eps:
                    rank_zero_info(
                        f"\nUpdate finite_difference_eps to {grid_size} at step {global_step}"
                    )
