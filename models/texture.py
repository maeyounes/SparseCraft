...
class DisentangledVolumeRadiance(nn.Module):
    def __init__(self, config):
        ...
        self.albedo_n_input_dims = (
            self.config.input_feature_dim - 3
        ...
        )  # point locations, SDF MLP features, view direction
    ...
    def forward_albedo(self, features):
        albedo_network_inp = torch.cat(
            [features.view(-1, features.shape[-1])],
            dim=-1,
        )
    ...
    def forward_specular(self, features, dirs, *args):
        dirs = (dirs + 1.0) / 2.0  # (-1, 1) => (0, 1)
        dirs_embd = self.encoding(dirs.view(-1, self.n_dir_dims))
        specular_network_inp = torch.cat(
            [features.view(-1, features.shape[-1]), dirs_embd]
    ...
    def output_albedo(self, features):
        albedo_network_inp = torch.cat(
            [features.view(-1, features.shape[-1])],
            dim=-1,
        )
    ...
    def output_specular(self, features, dirs, *args):
        dirs = (dirs + 1.0) / 2.0  # (-1, 1) => (0, 1)
        dirs_embd = self.encoding(dirs.view(-1, self.n_dir_dims))
        specular_network_inp = torch.cat(
            [features.view(-1, features.shape[-1]), dirs_embd]
