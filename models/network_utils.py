...
class ProgressiveBandHashGrid(nn.Module):
    def __init__(self, in_channels, config):
        ...
        if encoding_config.get("finest_resolution", None):
            ...
            encoding_config["per_level_scale"] = np.exp2(
                np.log2(finest_resolution / base_resolution) / (n_levels - 1)
            )  # finest_resolution = base_resolution * per_level_scale ** (n_levels - 1)
    ...
    def update_step(self, epoch, global_step):
        ...
        if current_level > self.current_level:
            rank_zero_info(
                f"\nUpdate grid level to {current_level} at iteration {global_step}"
            )
...
class CompositeEncoding(nn.Module):
    ...
    def forward(self, x, *args):
        encoding = self.encoding(x, *args)
        return (
            encoding
            if not self.include_xyz
            else torch.cat([x * self.xyz_scale + self.xyz_offset, encoding], dim=-1)
        )
...
def get_encoding(n_input_dims, config):
    ...
    if config.otype == "ProgressiveBandHashGrid":
        encoding = ProgressiveBandHashGrid(n_input_dims, config_to_primitive(config))
    else:
        with torch.cuda.device(get_rank()):
            if config.get("finest_resolution", None):
                finest_resolution = config["finest_resolution"]
                base_resolution = config["base_resolution"]
                n_levels = config["n_levels"]
                config["per_level_scale"] = float(
                    np.exp2(
                        np.log2(finest_resolution / base_resolution) / (n_levels - 1)
                    )
                )  # finest_resolution = base_resolution * per_level_scale ** (n_levels - 1)
...
class VanillaMLP(nn.Module):
    def __init__(self, dim_in, dim_out, config):
        ...
        self.sphere_init, self.weight_norm, self.invert = config.get(
            "sphere_init", False
        ), config.get("weight_norm", False), config.get("invert", False)
    ...
    def make_activation(self):
        if self.sphere_init:
            ...
            if self.config["activation"] == "Softplus":
                return nn.Softplus(beta=100)
            else:
...
def get_encoding_with_network(
    ...
    # input suppose to be range [0, 1]
    if encoding_config.otype == "ProgressiveBandHashGrid" or network_config.otype == "VanillaMLP":
