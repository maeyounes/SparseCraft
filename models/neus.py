...
class NeuSModel(BaseModel):
    def setup(self):
        ...
        if self.config.learned_background:
            ...
            self.near_plane_bg, self.far_plane_bg = self.config.near_plane_bg, self.config.far_plane_bg  # defaults are 0.1, 10 #1e2
            self.cone_angle_bg = (
                10
    ...
    def forward_(self, rays):
        ...
        space_points = (
            (torch.rand((5000, 3), device=self.rank) - 0.5) * 2 * self.config.radius
        )
        ...
        sdf, feature, sdf_grad = sdf[:-5000], feature[:-5000], sdf_grad_all[:-5000]
        ...
        out_full = {
            "comp_rgb": out["comp_rgb"] + out_bg["comp_rgb"] * (1.0 - out["opacity"]) if self.config.learned_background else out["comp_rgb"],
